...
#include "MySpartaLog/Actor/MyActor.h"
...
void AModuleAndPluginCharacter::Interact()
{
	if (IsValid(MyActor) == true)
	{
		FVector SpawnedLocation = (GetActorLocation() + GetActorForwardVector() * 300.f);
		AMyActor* SpawnedActor = GetWorld()->SpawnActor<AMyActor>(MyActor, SpawnedLocation, FRotator::ZeroRotator);
	}
}
